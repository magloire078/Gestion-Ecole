...
// Mock types and function to avoid build errors
export type GenerateTeacherRecommendationsInput = any;
export type GenerateTeacherRecommendationsOutput = any;

export async function generateTeacherRecommendations(
  input: GenerateTeacherRecommendationsInput
): Promise<GenerateTeacherRecommendationsOutput> {
    console.log("generateTeacherRecommendations is currently disabled.");
    return {
        recommendationLetterDraft: "La génération de lettre de recommandation est temporairement désactivée."
    };
}

    ...
    const handleAddIncident = async (values: IncidentFormValues) => {
        ...
        try {
            await DisciplineService.createIncident(
                ...
                studentInfo.id,
                incidentData,
                { uid: user.authUser.uid, displayName: user.authUser.displayName || undefined }
            );

            // Notify parents
            if (studentInfo.parentIds && studentInfo.parentIds.length > 0) {
                await DisciplineService.createParentNotifications(
                    schoolId,
                    studentInfo.parentIds,
                    { id: studentInfo.id, firstName: studentInfo.firstName },
                    values.type
                );
            }

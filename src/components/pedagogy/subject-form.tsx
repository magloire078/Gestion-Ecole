...
export function SubjectForm({ schoolId, subject, onSave }: SubjectFormProps) {
    ...
    const handleFormSubmit = async (values: SubjectFormValues) => {
        ...
        try {
            if (subject) {
                await SubjectsService.updateSubject(schoolId, subject.id, values);
            } else {
                await SubjectsService.createSubject(schoolId, values as unknown as Omit<Subject, 'id' | 'schoolId'>);
            }

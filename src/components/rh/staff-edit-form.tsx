...
export function StaffEditForm({ schoolId, editingStaff, classes, adminRoles, subjects, onFormSubmit }: StaffEditFormProps) {
    ...
    const handleSubmit = async (values: StaffFormValues) => {
        ...
        const dataToSave: Staff = {
            ...
            situationMatrimoniale: values.situationMatrimoniale as Staff['situationMatrimoniale'],
            adminRole: values.adminRole === 'none' ? undefined : values.adminRole,
        ...
        try {
            if (editingStaff) {
                ...
                await StaffService.updateStaffMember(schoolId, editingStaff.id, dataToSave);
                    ...
                    <Tabs defaultValue="general" className="w-full">
                        ...
                        <div className="py-6 max-h-[60vh] overflow-y-auto px-1">
                            <TabsContent value="general" className="mt-0 space-y-4">
                                ...
                                <FormField control={form.control} name="adminRole" render={({ field }) => (
                                    <FormItem><FormLabel>Rôle Administratif (Permissions)</FormLabel>
                                        <Select onValueChange={field.onChange} value={field.value || 'none'}>
                                            <FormControl><SelectTrigger><SelectValue placeholder="Aucun rôle admin" /></SelectTrigger></FormControl>
                                            <SelectContent>
                                                <SelectItem value="none">Aucun</SelectItem>
                                                {adminRoles.map(r => <SelectItem key={r.id} value={r.id}>{r.name}</SelectItem>)}
                                            </SelectContent>
                                        </Select>
                                    </FormItem>
                                )} />

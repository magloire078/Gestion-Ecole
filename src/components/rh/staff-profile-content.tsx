...
interface StaffProfileContentProps {
    staffId: string;
    /** URL vers la fiche de renseignements imprimable (schéma d'URL propre à chaque point d'entrée). */
    ficheHref: string;
    /** URL vers le bulletin de paie (schéma d'URL propre à chaque point d'entrée). */
    bulletinHref: string;
}

export function StaffProfileContent({ staffId, ficheHref, bulletinHref }: StaffProfileContentProps) {
        ...
        <>
            <div className="space-y-6">
                <div className="flex flex-wrap justify-end items-center gap-2">
                    <Button variant="outline" onClick={() => router.push(ficheHref)}>
                        <span className="flex items-center gap-2"><FileText className="mr-2 h-4 w-4" />Fiche de Renseignements</span>
                    </Button>
                    <Button variant="outline" onClick={() => router.push(bulletinHref)}>

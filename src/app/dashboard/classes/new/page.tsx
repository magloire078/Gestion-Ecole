...
import { collection, addDoc, doc, setDoc, deleteDoc, query, where, getDocs, limit, writeBatch, serverTimestamp } from "firebase/firestore";
...
export default function ClassesPage() {
  ...
  const form = useForm<ClassFormValues>({
    ...
    defaultValues: { cycle: '', grade: '', name: '', filiere: '', building: '', mainTeacherId: '' },
  });
  ...
  useEffect(() => {
    if (isFormOpen) {
      if (editingClass) {
        form.reset({
          cycle: editingClass.cycle, grade: editingClass.grade, name: editingClass.name, filiere: editingClass.filiere || '', building: editingClass.building, mainTeacherId: editingClass.mainTeacherId,
        });
      } else {
        form.reset({ cycle: '', grade: '', name: '', filiere: '', building: '', mainTeacherId: '' });
      }
    }
  }, [isFormOpen, editingClass, form]);
  
  const getMainTeacher = (teacherId?: string) => teachers.find(t => t.id === teacherId);
  ...
  const handleClassSubmit = async (values: ClassFormValues) => {
    ...
    const classData = {
        schoolId, cycle: values.cycle, grade: values.grade, name: values.name,
        filiere: values.cycle === "Enseignement Supérieur" ? values.filiere : "",
        building: values.building, mainTeacherId: values.mainTeacherId,
    ...
    try {
        if(editingClass) {
            ...
            toast({ title: "Classe créée", description: `La classe ${values.name} a été créée avec succès.` });
        }
        setIsFormOpen(false);
        setEditingClass(null);
    } catch(serverError) {
        const path = editingClass ? `ecoles/${schoolId}/classes/${editingClass.id}` : `ecoles/${schoolId}/classes`;
        const permissionError = new FirestorePermissionError({
            path, operation: editingClass ? 'update' : 'create', requestResourceData: classData
        });
  ...
  const handleCreateTeacher = async () => {
    ...
    const newTeacherData = { 
        schoolId, firstName: newTeacherFirstName, lastName: newTeacherLastName, role: 'enseignant',
        subject: newTeacherSubject, email: newTeacherEmail,
  ...
  return (
    <>
      <div className="space-y-6">
                ...
                <TabsContent key={cycle.order} value={cycle.name}>
                    <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
                     ...
                     {classes.filter(c => c.cycle === cycle.name).length === 0 && (
                        <div className="text-center text-muted-foreground py-10"> Aucune classe n'a été créée pour le cycle {cycle.name}. </div>
                    )}
                </TabsContent>
            ))}
        </Tabs>
       ...
       <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
          <DialogContent className="sm:max-w-xl">
            <DialogHeader>
              <DialogTitle>{editingClass ? "Modifier la" : "Créer une"} Classe</DialogTitle>
              <DialogDescription>
       ...
       <Dialog open={isAddTeacherDialogOpen} onOpenChange={setIsAddTeacherDialogOpen}>
            <DialogContent>
                <DialogHeader>
                    ...
                    <DialogDescription> L'enseignant n'a pas été trouvé. Veuillez fournir les détails requis. </DialogDescription>
                </DialogHeader>
                <div className="grid gap-4 py-4">
                    <div className="grid grid-cols-4 items-center gap-4"> <FormLabel htmlFor="new-teacher-firstname" className="text-right">Prénom</FormLabel> <Input id="new-teacher-firstname" value={newTeacherFirstName} onChange={e => setNewTeacherFirstName(e.target.value)} className="col-span-3" /> </div>
                    <div className="grid grid-cols-4 items-center gap-4"> <FormLabel htmlFor="new-teacher-lastname" className="text-right">Nom</FormLabel> <Input id="new-teacher-lastname" value={newTeacherLastName} onChange={e => setNewTeacherLastName(e.target.value)} className="col-span-3" /> </div>
                    <div className="grid grid-cols-4 items-center gap-4"> <FormLabel htmlFor="new-teacher-subject" className="text-right">Matière principale</FormLabel> <Input id="new-teacher-subject" value={newTeacherSubject} onChange={(e) => setNewTeacherSubject(e.target.value)} className="col-span-3" placeholder="Ex: Mathématiques" /> </div>
                    <div className="grid grid-cols-4 items-center gap-4"> <FormLabel htmlFor="new-teacher-email" className="text-right">Email</FormLabel> <Input id="new-teacher-email" type="email" value={newTeacherEmail} onChange={(e) => setNewTeacherEmail(e.target.value)} className="col-span-3" placeholder="Ex: prof@ecole.com" /> </div>
                </div>
                <DialogFooter>
      ...
      <AlertDialog open={isDeleteErrorAlertOpen} onOpenChange={setIsDeleteErrorAlertOpen}>
        <AlertDialogContent>
            <AlertDialogHeader>
                <AlertDialogTitle className="flex items-center gap-2"> <AlertTriangle className="text-destructive" /> Impossible de supprimer la classe </AlertDialogTitle>
                <AlertDialogDescription> Cette classe ne peut pas être supprimée car elle contient encore des élèves. Veuillez d'abord réaffecter tous les élèves à d'autres classes avant de procéder à la suppression. </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>

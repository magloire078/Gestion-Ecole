...
export default function BusManagementPage() {
  ...
  const handleDeleteBus = async () => {
    ...
    try {
      const routesQuery = query(collection(firestore, `ecoles/${schoolId}/transport_lignes`), where('busId', '==', busToDelete.id));
      const routesSnapshot = await getDocs(routesQuery);
      if (!routesSnapshot.empty) {
          toast({ variant: 'destructive', title: 'Action impossible', description: 'Ce bus est assigné à une ou plusieurs lignes. Veuillez d\'abord modifier ces lignes.' });
          return;
      }

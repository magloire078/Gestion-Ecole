  ...
  const inscriptions = useMemo(() => {
    ...
    return inscriptionsData.map(doc => {
      ...
      return {
        id: doc.id,
        ...data,
        studentName: studentMap.get(data.studentId) || 'N/A',
        activiteName: activiteMap.get(data.activiteId) || 'N/A',
      };
    });
  ...
  return (
    <>
      <Card>
        <CardHeader>
          <div className="flex justify-between items-center">
            <div>
              ...
              <CardDescription>Gérez les élèves inscrits aux activités parascolaires.</CardDescription>
            </div>
            {canManageContent && (
        ...
        <CardContent>
          <Table>
            <TableHeader><TableRow><TableHead>Élève</TableHead><TableHead>Activité</TableHead><TableHead>Année Scolaire</TableHead><TableHead className="text-right">Actions</TableHead></TableRow></TableHeader>
            <TableBody>
              {isLoading ? (
                ...
                inscriptions.map(inscription => (
                  <TableRow key={inscription.id}>
                    ...
                    <TableCell className="text-right">
                      {canManageContent && <Button variant="ghost" size="icon" onClick={() => handleDelete(inscription.id)}><Trash2 className="h-4 w-4 text-destructive" /></Button>}
                    </TableCell>
                  </TableRow>
                ))
              ) : (
                <TableRow><TableCell colSpan={4} className="h-24 text-center">Aucune inscription pour le moment.</TableCell></TableRow>
              )}
            </TableBody>
          </Table>
        </CardContent>
      ...
      <Dialog open={isFormOpen} onOpenChange={setIsFormOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>Inscrire un élève à une activité</DialogTitle>
          </DialogHeader>
          <Form {...form}>
            <form id="inscription-form" onSubmit={form.handleSubmit(handleFormSubmit)} className="space-y-4">
              <FormField control={form.control} name="studentId" render={({ field }) => <FormItem><FormLabel>Élève</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Choisir un élève..."/></SelectTrigger></FormControl><SelectContent>{students.map(s => <SelectItem key={s.id} value={s.id}>{s.firstName} {s.lastName}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>} />
              <FormField control={form.control} name="activiteId" render={({ field }) => <FormItem><FormLabel>Activité</FormLabel><Select onValueChange={field.onChange} value={field.value}><FormControl><SelectTrigger><SelectValue placeholder="Choisir une activité..."/></SelectTrigger></FormControl><SelectContent>{activites.map(a => <SelectItem key={a.id} value={a.id}>{a.name}</SelectItem>)}</SelectContent></Select><FormMessage /></FormItem>} />
          ...
          <DialogFooter>
            ...
            <Button type="submit" form="inscription-form" disabled={form.formState.isSubmitting}>Inscrire</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>

...
export default function OnboardingPage() {
  ...
  const handleJoinSchool = async () => {
    ...
    try {
      ...
      const q = query(schoolsRef, where('code', '==', schoolCode.trim().toUpperCase()));
      ...
      const memberRef = doc(firestore, `schools/${schoolId}/members`, authUser.uid);
      ...
      batch.set(memberRef, {
        userId: authUser.uid,
        role: 'teacher', // Rôle par défaut
        joinedAt: new Date().toISOString(),
      });

      await batch.commit();

      await authUser.getIdToken(true); // Force refresh token
